...
namespace Hercules {
    void Hercules::Model::Draw(Texture& texture, glm::vec3& pos, glm::vec3& scale,
        glm::vec3& rotation, Shader* shader)
    {
        for (unsigned int i = 0; i < meshes.size(); i++)
            meshes[i].Draw(texture, pos, scale, rotation, shader);
    }

    unsigned int Hercules::Model::TextureFromFile(const char* path, const std::string& directory, bool gamma)
    {
    ...
    void Hercules::Model::loadModel(std::string path)
    {
        ...
        {
            HC_CORE_TRACE("Assimp error: {0}", importer.GetErrorString());
            return;
        }
        directory = path.substr(0, path.find_last_of('/'));
        //HC_CORE_TRACE(directory);
